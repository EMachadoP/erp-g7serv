...
<div class="container mobile-container mt-3 pb-5 mb-5">
    ...
    <div id="actions-area" class="d-grid gap-3">
        ...
        <button id="btn-checkin" class="btn btn-success btn-lg shadow" onclick="performCheckin()">
            <i class="fas fa-map-marked-alt"></i> Iniciar (Check-in)
        </button>
        
        <!-- Mock GPS for development -->
        <script>
            if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
                document.write('<button class="btn btn-outline-warning btn-sm mt-2" onclick="mockCheckin()"><i class="fas fa-flask"></i> Simular GPS (Apenas PC)</button>');
            }
            function mockCheckin() {
                ...
                .then(r => r.json())
                .then(d => { hideLoading(); if(d.success) location.reload(); else alert(d.error); })
                .catch(e => { hideLoading(); alert("Erro na simulação"); });
            }
        </script>
