...
<div class="container mobile-container mt-3 pb-5 mb-5">
    ...
    <div id="actions-area" class="d-grid gap-3">
        ...
        <!-- Mock GPS tool (Enhanced) -->
        <script>
            if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
                document.write(`
                    <div class="alert alert-warning border shadow-sm p-3 mb-2 text-center" style="position: relative; z-index: 10001;">
                        <h6 class="mb-1"><i class="fas fa-flask"></i> FERRAMENTA DE TESTE (PC)</h6>
                        <button class="btn btn-warning w-100 shadow-sm fw-bold mt-2" onclick="mockCheckin()">
                            SIMULAR GPS E INICIAR AGORA
                        </button>
                    </div>
                `);
            }
        ...
        <button id="btn-checkin" class="btn btn-success btn-lg shadow py-3" onclick="performCheckin()">
    ...
    <!-- Gallery Preview -->
    <div id="gallery-area" class="row mt-4 g-2">
...
<div id="loading"
    ...
    <div class="text-light mb-3">Buscando localização...</div>
    <button class="btn btn-outline-light btn-sm" onclick="hideLoading()">
        <i class="fas fa-times"></i> Cancelar e Usar Simulador
    </button>
</div>
    ...
    function performCheckin() {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (error) => {
                ...
                if (error.code === 1) msg = "Acesso ao GPS negado.";
                else if (error.code === 2) msg = "GPS não disponível.";
                else if (error.code === 3) msg = "Tempo esgotado.";
                alert(msg + " Tente usar o botão de Simulação se estiver no PC.");
            },
            {
...
print(f"File updated with Cancel option: {path}")
